...
from regex_engine import Regex  # Убедись, что здесь импортируется актуальный Regex/DFA
import re
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            if not regex:
                print("⚠️ Сначала скомпилируйте выражение.")
                continue
            ...
            if not regex or regex.pattern is None:
                print("⚠️ Нельзя сравнить — нет оригинала.")
                continue
            restored = regex.dfa.to_regex()
